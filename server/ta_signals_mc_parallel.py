    ...
    try:
        with getDbConnection() as con:
            ...
            for col, ctype in schema.items():
                ...
                char_max = None
                try:
                    res = con.execute(q_check, [table_name, col])
                    row = res.fetchone()
                    if row:
                        data_type = row[0]
                        char_max = row[1]
            ...
            for idx_name, cols in idx_defs.items():
                ...
                """
                try:
                    res = con.execute(q_idx_check, [table_name, idx_name])
                    r = res.fetchone()
...
def get_historic_prices_from_finnhub_local(symbol: str) -> pd.DataFrame:
    with getDbConnection() as con:
        ...
        ORDER BY date ASC
        """
        df = pd.read_sql(q, con=con, params=[symbol])
    return df
...
def get_ADX_Strength(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    df['ADX_Strength'] = np.select(conditions, choices)
    return df
...
def get_DI_Strength(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    choices = ['Strong', 'Average', 'Weak']
    df['DI_Strength'] = np.select(conditions, choices)
    return df
...
def get_symbols_forwatchlist(watchlist: str, config: dict) -> pd.DataFrame:
    with getDbConnection() as conn:
        query = f"SELECT DISTINCT symbol FROM {config['WATCHLIST_TABLENAME']} WHERE watchlist = %s;"
        df = pd.read_sql(query, con=conn, params=[watchlist])
    return df
...
def main():
    ...
    try:
        ...
        delete_sql = f'DELETE FROM `{config["tal_temp_tablename"]}` WHERE `CountryName` = %s'
        with getDbConnection() as con:
            con.execute(delete_sql, [countryName])
            try:
                if hasattr(con, 'commit') and callable(con.commit):
                    con.commit()
            except Exception:
    ...
    if not df_final.empty:
        try:
            with getDbConnection() as con:
                sql_delete_master = f'DELETE FROM `{config["tal_master_tablename"]}` WHERE `CountryName` = %s'
                result = con.execute(sql_delete_master, [countryName])
                try:
                    if hasattr(con, 'commit') and callable(con.commit):
                        con.commit()
                except Exception:
